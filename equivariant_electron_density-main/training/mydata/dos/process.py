...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        np.savetxt((os.path.join(data_path, structure_id + "_processed_0.csv")),
                   np.concatenate((dos_read[0, 0, :][np.newaxis, :], dos_sum), axis=0), delimiter=",")
    ...
    if processing_args["LMBTR_descriptor"] == "True":
        ...
        make_feature_LMBTR = LMBTR(
            ...
            k2={"geometry": {"function": "distance"},
                "grid": {"min": 0, "max": processing_args["LMBTR_rcut"], "sigma": processing_args["LMBTR_sigma"],
                         "n": processing_args["LMBTR_grid"]},
                "weighting": {"function": "exp", "scale": 0.5, "threshold": 1e-3}},
            k3={"geometry": {"function": "angle"},
                "grid": {"min": 0, "max": 180, "sigma": processing_args["LMBTR_sigma"],
                         "n": processing_args["LMBTR_grid"]},
                "weighting": {"function": "exp", "scale": 0.5, "threshold": 1e-3}},
            periodic=True,
            sparse=False,
...
##get dos features
def get_dos_features(x, dos,spd=True):
    dos = torch.abs(dos)
    if spd == True:
        ...
        center = torch.sum(x * dos, axis=1) / torch.sum(dos, axis=1)
        ef_states = torch.sum(dos[:, zero_index - 20:zero_index + 20], axis=1) * abs(x[0] - x[1])
        # return torch.stack((center, ef_states), axis=1)
        return ef_states
...
def get_dos_features_5(x, dos):
    ...
    # return torch.stack((center, width, ef_states), axis=1)
